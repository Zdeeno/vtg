...
def plot_samples_lowdim(source, repr, name=0, dir="results/0/"):
    fig, axarr = plt.subplots(2)
    axarr[0].imshow(source.permute(1, 2, 0), aspect="auto")
    rh, rw = repr.shape[-2], repr.shape[-1]
    h, w = 384, 512
    dims = repr.shape[0]
    out = t.zeros(dims, 192, 512)
    for i in range(dims):
        f = interpolate.interp2d(np.linspace(0, w, rw), np.linspace(0, h, rh), repr[i], kind="linear")
        interpolated = f(np.arange(512), np.arange(192))
        out[i] = t.tensor(interpolated)
    axarr[1].imshow(out.permute(1, 2, 0), aspect="auto")
...
def plot_displacement(source, target, prediction, displacement=None, importance=None, importance_shift=None,
                      name=0, dir="results/0/"):
    if importance is not None and importance_shift is not None:
        # get shifted target
        ...
        source_shifted = t.roll(source, predicted_shift, -1)
        # plot
        importance = importance
        importance = t.roll(t.tensor(importance), importance_shift * 8, -1).numpy()
        out, axs = plt.subplots(2)
        h = sns.heatmap(importance, alpha=0.5, zorder=2, cbar=False, ax=axs[0])
        h.imshow(target.permute(1, 2, 0))
        h2 = axs[1].imshow(source_shifted.permute(1, 2, 0))
        plt.savefig(dir + str(name) + ".png")
        plt.close()
        return
...
def plot_similarity(img1, img2, time_histogram, name=None, offset=None, idx_shift=None):
    f, axarr = plt.subplots(3)
    ...
    axarr[2].set_xlabel("offset from $j_k$ [frames]")
