...
def create_conv_block(in_channel, out_channel, kernel, stride, padding, pooling, bn=True, relu=True,
                      pool_layer=False):
    net_list = [t.nn.Conv2d(in_channel, out_channel, kernel, stride, padding, padding_mode="zeros")]
    if bn:
...
class CNNHead(t.nn.Module):

    def __init__(self):
        super().__init__()
        self.layers = t.nn.Sequential(*[create_conv_block(256, 64, 1, 1, 0, (1, 1), pool_layer=False),
                                        create_conv_block(64, 16, 1, 1, 0, (1, 1), pool_layer=False),
                                        create_conv_block(16, 1, 1, 1, 0, (1, 1), relu=False, pool_layer=False)])
...
class CNNOLD(t.nn.Module):
    ...
    def __init__(self, lp, fs, ech, res, legacy=True):
        ...
        #                                   create_conv_block(512, 256, 3, 1, 1, (2, 1), bn=False, relu=False, pool_layer=False)])
        ...
        self.l5 = create_conv_block(512, ech, fs, 1, pad, (3, 1), relu=False, bn=False, pool_layer=lp)
...
class Siamese(t.nn.Module):
    ...
    def forward(self, source, target, padding=None, displac=None, repr=None):
        if repr is None:
