...
DATASET_NAME = "cestlice_reduced"
DATASET_PLOT = "cestlice"
ERROR_CAP = 512
...
def error_distribution(files: [str], names: [str]):
    ...
    for file in files:
        if len(file) > 1 and file[1]:
            array = np.genfromtxt(file[0], delimiter=',')[:450]  # - diff
        else:
    ...
    plt.title("Accuracies on " + DATASET_NAME + " dataset", fontsize=22)
    plt.grid()
    plt.xlabel("Registration error threshold [px]", fontsize=20)
    plt.ylabel("Prob. of correct registration [-]", fontsize=20)
    # plt.xscale("log")
    plt.xlim([0, 100])
    plt.ylim([0.6, 1.001])
...
def plot_cutout_region(img_path1: str, img_path2: str, region1: [[int], [int]], region2: [[int], [int]]):
    ...
    # print(rect)
    # rect = rect + np.random.rand(rect.shape)
